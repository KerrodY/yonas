...
module PlayerBuilds
  ...
  def self.format_builds_table(builds)
    ...
    builds.each do |build|
      message += "#{build.player.ljust(16)} | #{build.role.ljust(10)} | #{build.weapon1.ljust(16)} | #{build.weapon2.ljust(16)} | #{build.armour_weight.ljust(6)} | #{build.heartrune.ljust(14)}\n"
    end
  ...
  application_command(:register_pvp_build) do |event|
    ...
    PlayerBuild.find_or_initialize_by(discord_id: event.user.username, server_id: event.server.id).update!(
      ...
      weapon1: event.options['weapon1'],
      weapon2: event.options['weapon2'],
    ...
    event.user.add_role(event.server.roles.find { |r| r.name == event.options['weapon1'] })
    event.user.add_role(event.server.roles.find { |r| r.name == event.options['weapon2'] })
  ...
  application_command(:export_pvp_builds) do |event|
    ...
    csv_data = CSV.generate(headers: true) do |csv|
      ...
      builds.each do |build|
        csv << [build.player, build.role, build.weapon1, build.weapon2, build.armour_weight, build.heartrune, build.guest, build.discord_id]
      end
    end
  ...
  def self.register_commands(bot, server_id:)
    bot.register_application_command(:register_pvp_build, 'Register your PvP war build', server_id:) do |cmd|
      ...
      cmd.string('weapon1', 'your first weapon', required: true, choices: DATA::WEAPONS_PARAMS)
      cmd.string('weapon2', 'your second weapon', required: true, choices: DATA::WEAPONS_PARAMS)
    ...
    bot.register_application_command(:delete_pvp_build, 'Delete your PvP war build', server_id:)
    ...
    bot.register_application_command(:export_pvp_builds, 'Export all PvP builds to a CSV file', server_id:)

    bot.register_application_command(:unregistered_pvp_builds, 'Show all registered players pvp builds', server_id:)
    ...
    bot.register_application_command(:search_pvp_builds, 'Select weapons and/or armour weight to search for players using them', server_id:) do |cmd|
      ...
      cmd.string('weapon1', 'first weapon', choices: DATA::WEAPONS_PARAMS)
      cmd.string('weapon2', 'second weapon', choices: DATA::WEAPONS_PARAMS)
